...
* 测试前确认已安装MySQL数据库
    ...
    // 添加数据
    INSERT INTO user(username, passwd) VALUES('name', 'passwd'); //name:root passwd:Abcdefg@123
    ```
...
* 修改main.c中的数据库初始化信息
    ...
    connPool->init("localhost", "root", "root", "yourdb", 3306, 8); //connPool->init("localhost", "root", "Abcdefg@123", "yourdb", 3306, 8);
    ```
...
* 生成server
    修改makefile，//将-lmysqlclient修改为-L/usr/lib64/mysql -lmysqlclient
    ```C++
    make server
    ```
